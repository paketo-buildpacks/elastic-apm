...
func (n NodeJSAgent) Contribute(layer libcnb.Layer) (libcnb.Layer, error) {
	...
	if !regexp.MustCompile(`require\(['"]elastic-apm-node['"]\)`).Match(c) {
		n.Logger.Header("Requiring 'elastic-apm-node' module")

		if err := ioutil.WriteFile(file, append([]byte("require('elastic-apm-node').start();\n"), c...), 0644); err != nil {
